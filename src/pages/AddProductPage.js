...
const handleInvalid = (e) => {
  ...
  const handleSubmit = (e) => {
    ...
    // productToSend.
      ...
      if (product.productType === 'DVD') {
          productToSend.size = value;
      } else if (product.productType === 'Book') {
          productToSend.weight = value;
      } else if (product.productType === 'Furniture') {
          productToSend.dimensions = value;
      }

      const form_data = new FormData();

      for ( let key in productToSend ) {
          form_data.append(key, productToSend[key]);
      }

    fetch('https://nika-scandi-assignment.000webhostapp.com/api/products/add', {
      method: 'POST',
      body: form_data,
    })
    .then(response => {
